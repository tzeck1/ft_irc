...
/*   Updated: 2022/11/10 10:16:49 by btenzlin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Server::receive_msg(int i)
{
	...
	// std::cout << "RECEIVE: " << _recv_msg << std::endl;
	/* *TESTING* */
	// User user = welcome_msg(_recv_msg, _socket_addr.sin_addr.s_addr); //throws exception!
	// std::string welcome = user.build_reply();
	// send(_client_fds[i].fd, welcome.c_str(), 94, 0);
	/* *TESTING* */
